...
public interface TodoMapper {
	...
	@Update("update todo set tcontent=#{tcontent}, latitude=#{latitude}, longitude=#{longitude},"
			+ " tododate=TO_TIMESTAMP(#{tododate},'YYYY.MM.DD HH24:MI'), priority=#{priority}, sharing=#{sharing} where tseq=#{tseq}")
	int updateTodo(TodoVO vo);
	
	@Select("select tseq, tcontent, latitude, longitude, to_char(tododate,'yyyy.mm.dd hh24:mi') as tododate, priority, sharing, id from todo where tseq=#{tseq}")
	TodoVO getTodo(int tseq);
	
	@Select("select tseq, tcontent, latitude, longitude, to_char(tododate,'yyyy.mm.dd hh24:mi') as tododate, priority, sharing, id from todo where id=#{id} and tododate between to_date(#{tododate}, 'YYYYMMDD') and to_date(#{tododate}, 'YYYYMMDD')+1")
